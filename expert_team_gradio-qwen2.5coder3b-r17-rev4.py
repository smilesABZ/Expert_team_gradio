...
iface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Radio(temperature_presets.keys(), label="Approach style for Expert 1", info="Set approach for Expert 1 (Cautious, Balanced, Creative)."), # Modified to radio buttons
        gr.Radio(temperature_presets.keys(), label="Approach style  Role 2", info="Set approach for Expert 2."),
        gr.Radio(temperature_presets.keys(), label="Approach style  Role 3", info="Set approach for Expert 3."),
        gr.Radio(temperature_presets.keys(), label="Approach style  Role 4", info="Set approach for Expert 4."),
        gr.Radio(evaluation_model_options.keys(), label="Evaluation Model", info="Choose the model for senior manager evaluation."), # Specific evaluation model dropdown
    ],
    outputs=[
        gr.Textbox(label="Response from Expert 1"), # More descriptive output labels
        gr.Radio(temperature_presets.keys(), label="Approach style for Expert 2"),
        gr.Radio(temperature_presets.keys(), label="Approach style  Role 3"),
        gr.Radio(temperature_presets.keys(), label="Approach style  Role 4"),
