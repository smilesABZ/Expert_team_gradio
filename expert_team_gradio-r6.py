...
async def generate_responses(question, role1_name, role2_name, role3_name, role4_name):
    ...
    return responses.get(role1_name, "No Response"), responses.get(role2_name, "No Response"), responses.get(role3_name, "No Response"), responses.get(role4_name, "No Response"), responses
...
# Gradio interface (using asyncio) - ***CRUCIAL: ASYNC FUNCTION CALLING GENERATE_RESPONSES***
async def gradio_interface(question, role1, role2, role3, role4): # ***CRUCIAL: 5 ARGUMENTS***
    llm1_response, llm2_response, llm3_response, llm4_response, all_responses = await generate_responses(question, role1, role2, role3, role4) # ***CRUCIAL: CALLING GENERATE_RESPONSES***
    evaluation, perfect_answer, illustration_prompt = await evaluate_responses(all_responses)
    return llm1_response, llm2_response, llm3_response, llm4_response, evaluation, perfect_answer, illustration_prompt


iface = gr.Interface(
    fn=gradio_interface, # ***CRUCIAL:  USE THE ASYNC gradio_interface FUNCTION***
    inputs=[
        "text",
    ...
    outputs=[
        gr.Textbox(label="LLM 1 Response"), # ***SIMPLIFIED LABELS - Easier to Verify***
